...
(function() {
	//#region src/shared/policy/download-controller.ts
	function resolveDownloadController(input) {
		if (input.seekUrgent || input.liveUrgent) return {
			nextConcurrency: input.maxConcurrency,
			nextLowGainStreak: 0
		};
		if (input.queueDepth <= 0) return {
			nextConcurrency: input.bufferedSeconds > input.targetBufferSeconds + 8 && input.cacheSatisfied ? input.minConcurrency : clamp$1(input.currentConcurrency, input.minConcurrency, input.maxConcurrency),
			nextLowGainStreak: 0
		};
		const throughputGain = input.previousThroughputBps > 0 ? (input.recentThroughputBps - input.previousThroughputBps) / input.previousThroughputBps : input.recentThroughputBps > 0 ? 1 : 0;
		if (input.recentErrorRate >= .05) return {
			nextConcurrency: clamp$1(input.currentConcurrency - 2, input.minConcurrency, input.maxConcurrency),
			nextLowGainStreak: 0
		};
		if (throughputGain >= .08 && input.recentErrorRate < .03) return {
			nextConcurrency: clamp$1(input.currentConcurrency + 2, input.minConcurrency, input.maxConcurrency),
			nextLowGainStreak: 0
		};
		const nextLowGainStreak = Math.abs(throughputGain) < .03 ? input.lowGainStreak + 1 : 0;
		if (nextLowGainStreak >= 3) return {
			nextConcurrency: clamp$1(input.currentConcurrency - 2, input.minConcurrency, input.maxConcurrency),
			nextLowGainStreak: 0
		};
		if (input.bufferedSeconds > input.targetBufferSeconds + 8 && input.cacheSatisfied) return {
			nextConcurrency: input.minConcurrency,
			nextLowGainStreak: 0
		};
		return {
			nextConcurrency: clamp$1(input.currentConcurrency, input.minConcurrency, input.maxConcurrency),
			nextLowGainStreak
		};
	}
	...
	//#endregion
	//#region src/shared/policy/live.ts
	var MB$1 = 1024 * 1024;
	function sortLiveCandidates(mode, candidates) {
		const chosen = mode === "lowLatency" ? [
			"flv",
			"fmp4",
			"ts"
		] : [
	...
	function priority(name, order) {
		const idx = order.findIndex((item) => item === name);
		return idx === -1 ? Number.MAX_SAFE_INTEGER : idx;
	}
	function codecScore(codecName) {
	...
	function parseLivePlaylistItems(baseUrl, text) {
		const results = [];
		...
		for (const line of text.split(/\r?\n/)) {
			...
			if (trimmed.startsWith("#")) {
				...
				if (mapMatch?.[1]) {
					...
					if (tagName === "EXT-X-MAP") results.push({
						url: uriUrl,
						kind: "media",
						durationSeconds: null
					});
					else if (tagName === "EXT-X-MEDIA" || tagName === "EXT-X-I-FRAME-STREAM-INF") results.push({
						url: uriUrl,
	...
	function resolveLivePrefetchWindow(input) {
		if (input.totalSegments <= 0) return 0;
		const target = Math.max(3, input.targetBufferSeconds);
		const avgSegmentSeconds = Math.max(.5, input.avgSegmentSeconds ?? 2);
		const desired = Math.ceil(target / avgSegmentSeconds);
		if (input.bufferedSeconds < 1.5) return Math.min(input.totalSegments, Math.max(4, desired * 2));
		const headroom = input.bufferedSeconds < target ? Math.ceil(desired * .5) : 1;
		return Math.min(input.totalSegments, Math.max(3, desired + headroom));
	}
	function resolveLiveCacheTargetBytes(input) {
		const targetSeconds = Math.max(3, input.targetBufferSeconds);
		const avgSegmentSeconds = Math.max(.5, input.avgSegmentSeconds ?? 2);
		const measuredBytesPerSecond = input.recentBytesPerRequest > 0 ? input.recentBytesPerRequest / avgSegmentSeconds : 0;
		const fallbackBps = (input.quality ?? 0) >= 1e4 ? 5e7 : 12e6;
		const bytesPerSecond = measuredBytesPerSecond > 0 ? measuredBytesPerSecond : fallbackBps / 8;
		return clampBytes$1(Math.round(bytesPerSecond * targetSeconds * 1.6), (input.quality ?? 0) >= 1e4 ? 64 * MB$1 : 32 * MB$1, (input.quality ?? 0) >= 1e4 ? 384 * MB$1 : 192 * MB$1);
	}
	function clampBytes$1(value, min, max) {
		if (!Number.isFinite(value)) return min;
		return Math.min(max, Math.max(min, value));
	}
	//#endregion
	//#region src/shared/policy/prefetch.ts
	var MB = 1024 * 1024;
	function resolvePrefetchDecision(input) {
		const highBitrateMode = (input.quality ?? 0) >= 1440 || (input.estimatedBitrate ?? 0) >= 8e6;
		const phase = input.phase ?? "steady";
		const baseWindow = 12;
		const configuredTargetSeconds = clamp(input.aggressivePrefetchSeconds, 48, 48);
		const remainingSeconds = Number.isFinite(input.remainingSeconds ?? NaN) ? Math.max(0, input.remainingSeconds ?? 0) : null;
		const targetSeconds = remainingSeconds != null && remainingSeconds > 0 ? Math.min(configuredTargetSeconds, remainingSeconds) : configuredTargetSeconds;
		const cacheEntireRemaining = remainingSeconds != null && remainingSeconds <= configuredTargetSeconds;
		const avgSegmentSeconds = Math.max(1, (input.avgSegmentDurationMs ?? 4e3) / 1e3);
		const futureSegments = Math.max(1, Math.ceil(targetSeconds / avgSegmentSeconds));
		const videoWindow = cacheEntireRemaining ? futureSegments : clamp(Math.max(baseWindow, futureSegments + (phase === "seek" ? 6 : phase === "initial" ? 3 : 0)), 8, phase === "seek" ? 30 : phase === "initial" ? 24 : 18);
		const audioWindow = cacheEntireRemaining ? videoWindow : clamp(videoWindow, 4, 18);
		const totalConcurrency = clamp(Math.max(input.maxConcurrentRequests, phase === "seek" ? 16 : phase === "initial" ? 14 : Math.min(videoWindow + 2, 12)), 8, 16);
		const bitrateEstimate = Math.max(25e5, input.estimatedBitrate ?? (highBitrateMode ? 12e6 : 5e6));
		const multiplier = phase === "seek" ? 2.1 : phase === "initial" ? 1.9 : 1.6;
		return {
			videoWindow,
			audioWindow,
			totalConcurrency,
			cacheLimitBytes: clampBytes(Math.round(bitrateEstimate / 8 * Math.max(targetSeconds, 12) * multiplier), highBitrateMode ? 256 * MB : 128 * MB, 1024 * MB),
			targetSeconds,
			highBitrateMode
		};
	}
	...
	//#endregion
	//#region src/shared/policy/request.ts
	function classifyResource(url) {
		if (/x\/player\/wbi\/playurl/.test(url)) return "playurl";
		if (/xlive\/web-room\/v2\/index\/getRoomPlayInfo/.test(url)) return "livePlayInfo";
		if (/\.m3u8(\?|$)/.test(url)) return "playlist";
		if (/\.m4s(\?|$)|\.mp4(\?|$)|\.flv(\?|$)|\.ts(\?|$)|\.cmfv(\?|$)/.test(url)) return "media";
		return "other";
	}
	function getRequestSemantics(input, init) {
		const request = typeof Request !== "undefined" && input instanceof Request ? input : null;
		const url = extractRequestUrl$1(input);
	...
	function shouldUseFullBodyCache(semantics, resourceKind) {
		return semantics.ordinaryGet && (resourceKind === "media" || resourceKind === "playlist");
	}
	function shouldDriveStreamingPipeline(semantics, resourceKind) {
		return semantics.ordinaryGet && (resourceKind === "media" || resourceKind === "playlist");
	}
	function shouldUseRangeSplitForRequest(input) {
		if (input.pageKind !== "vod" || input.mode === "off" || !input.experimentalRangeSplit) return false;
		if (!/\.m4s(\?|$)|\.mp4(\?|$)|\.cmfv(\?|$)/.test(input.url)) return false;
		return input.semantics.ordinaryGet;
	}
	function extractRequestUrl$1(input) {
		if (typeof input === "string") return input;
		if (typeof Request !== "undefined" && input instanceof Request) return input.url;
		return input.toString();
	}
	//#endregion
	//#region src/shared/policy/recovery.ts
	function selectRecoveryAction(input) {
		if (input.mode === "off") return "noop";
		if (input.seekInProgress) return "noop";
		if (input.bufferedSeconds < 1.25) return input.pageKind === "live" ? "rebuild-live-player" : "rebuild-playback-state";
		if (input.pageKind === "live" && input.enableProtocolFallback && input.repeatedStalls >= 2) return "switch-live-protocol";
		if (input.droppedFrames >= 12) return "prefer-stable-codec";
		if (input.hostFailures >= 2) return "drop-quality";
		return input.pageKind === "live" ? "rebuild-live-player" : "rebuild-playback-state";
	}
	//#endregion
	//#region src/page/index.ts
	var PAGE_MESSAGE_SOURCE = "bwf-page";
	var DEFAULT_POLICY = {
		mode: "stable",
		vod: {
			lockQuality: true,
	...
	var RECOVERY_LIMITS = {
		sameActionCooldownMs: 1e4,
		qualityDropCooldownMs: 3e4,
		rebuildCooldownMs: 6e4
	};
	var VOD_BACKGROUND_PUMP_INTERVAL_MS = 1e3;
	var nativeDocumentHiddenGetter = getDocumentGetter("hidden");
	var nativeDocumentVisibilityStateGetter = getDocumentGetter("visibilityState");
	var pageKind = detectPageKind(location.href);
	var policy = DEFAULT_POLICY;
	var telemetry = [];
	var overlay = null;
	var overlayBody = null;
	var overlayExpanded = true;
	var recoveryTimestamps = /* @__PURE__ */ new Map();
	var hostFailures = /* @__PURE__ */ new Map();
	var hostHealth = /* @__PURE__ */ new Map();
	var hostFailureSamples = [];
	var segmentDurations = [];
	var liveSegmentDurations = [];
	var mediaCache = /* @__PURE__ */ new Map();
	var inflightPrefetches = /* @__PURE__ */ new Map();
	var prefetchQueue = [];
	var liveSeenSegments = /* @__PURE__ */ new Set();
	var nativeFetch = window.fetch.bind(window);
	var inflightRangeRequests = /* @__PURE__ */ new Map();
	var inflightCacheWrites = /* @__PURE__ */ new Map();
	var inflightMediaResponses = /* @__PURE__ */ new Map();
	var activeMediaFetches = /* @__PURE__ */ new Set();
	var downloadSamples = [];
	var prefetchActive = 0;
	var cacheBytes = 0;
	var prefetchHitCount = 0;
	var rangeChunkRetryCount = 0;
	var lastMediaUrl = null;
	var lastVodPrefetchSeedUrl = null;
	var lastVodBackgroundPumpAt = 0;
	var lastLivePlaylistUrl = null;
	var lastLiveLeafPlaylistUrl = null;
	var lastLivePlaylistRefreshAt = 0;
	var inflightLivePlaylistRefresh = null;
	var activeRangeChunkJobs = 0;
	var lastBufferedRangeEnd = 0;
	...
	var vodSelection = {
		...
		durationSeconds: null,
		availableQualities: [],
		availableCodecsByQuality: /* @__PURE__ */ new Map(),
		currentTrackPaths: [],
		videoBaseUrl: null,
		audioBaseUrl: null
	};
	...
	try {
		...
		document.documentElement.dataset.bwfPageError = error instanceof Error ? `${error.name}:${error.message}` : String(error);
		throw error;
	}
	...
	function installFetchHook() {
		window.fetch = async (input, init) => {
			...
			if (cacheHit) {
				prefetchHitCount += 1;
				emitNetworkEvent("prefetch", requestUrl, 0, true, void 0, "cache-hit");
			...
			if (resourceKind === "media" && drivesStreamingPipeline) activeMediaFetches.add(requestUrl);
			let response;
			let finalUrl;
			let usedSplitResponse = false;
			try {
				const splitResponse = resourceKind === "media" ? await maybeServeMediaWithRangeSplit(requestUrl, input, init, requestSemantics) : null;
				...
				if (resourceKind === "media" && drivesStreamingPipeline) activeMediaFetches.delete(requestUrl);
			}
			const responseForPage = await maybeRewriteFetchResponse(requestUrl, response);
			const durationMs = Math.round(performance.now() - started);
			const clone = resourceKind === "playurl" || resourceKind === "livePlayInfo" || resourceKind === "playlist" ? responseForPage.clone() : null;
			if (drivesStreamingPipeline || resourceKind === "playurl" || resourceKind === "livePlayInfo") noteActiveMediaHost(finalUrl, resourceKind);
			...
			if (canUseFullBodyCache && resourceKind === "media" && responseForPage.ok) storeFetchedMediaResponse(finalUrl, responseForPage.clone());
			handleFetchResponse(finalUrl, resourceKind, clone ?? responseForPage, durationMs);
			...
			} else if (resourceKind === "playlist" && drivesStreamingPipeline && responseForPage.ok) scheduleLivePlaylistPrefetch(finalUrl, responseForPage.clone());
	...
	function installXhrHook() {
		...
		XMLHttpRequest.prototype.send = function send(body) {
			...
			this.addEventListener("loadend", () => {
				...
				emitNetworkEvent(classifyResource(url), url, duration, this.status >= 200 && this.status < 400, void 0, `xhr:${this.status}`);
				if (this.status >= 400) noteHostFailure(url);
			});
	...
	function installMediaHooks() {
		...
		const attach = () => {
			...
			video.addEventListener("timeupdate", () => {
				...
				if (seekState.active && state.bufferedSeconds > .75) resolveSeekTracking(video, "timeupdate");
				publishStatus();
			});
		};
	...
	function installVisibilityHooks() {
		document.addEventListener("visibilitychange", () => {
			...
			if (isPageActuallyHidden()) {
				if (pageKind === "vod") pumpVodBackgroundPrefetch(true);
				else if (pageKind === "live") maybeRefreshLivePlaylist(true);
			}
			publishStatus();
		});
	}
	function installVodVisibilityGuard() {
		if (pageKind !== "vod") return;
	...
	function spoofDocumentHidden() {
		return false;
	}
	function spoofDocumentVisibilityState() {
		return "visible";
	}
	function installMseHooks() {
		...
		if (!mediaSourceProto || !sourceBufferProto) return;
	...
	function patchSourceBuffer(buffer, mimeType) {
		...
		buffer.appendBuffer = ((data) => {
			...
			const finalize = (ok, detail) => {
				...
				if (ok) noteSegmentDuration(duration);
			};
			const onUpdateEnd = () => {
				cleanup();
	...
	async function handleFetchResponse(url, resourceKind, response, durationMs) {
		...
		const bytes = bytesHeader ? Number(bytesHeader) : void 0;
		emitNetworkEvent(resourceKind, url, durationMs, response.ok, bytes);
		if (resourceKind === "playurl" && response.ok) {
			const json = await response.json().catch(() => null);
			if (json) handleVodPlayurl(json);
		} else if (resourceKind === "livePlayInfo" && response.ok) {
			const json = await response.json().catch(() => null);
			if (json) handleLivePlayInfo(json);
		}
	}
	function handleVodPlayurl(payload) {
		const dash = (payload?.data?.dash ? payload.data : payload?.result?.video_info?.dash ? payload.result.video_info : null)?.dash;
		if (!dash) return;
		...
		const filteredVideos = preferredQuality != null ? videos.filter((item) => item.id === preferredQuality) : videos;
		const candidates = filteredVideos.length > 0 ? filteredVideos : videos;
		const top = rankCodecPreference(candidates.map((item) => ({
			id: item.id,
			codecs: item.codecs ?? "",
			bandwidth: item.bandwidth,
			width: item.width,
			height: item.height
		})), resolveVodCodecPreference())[0];
		...
		const availableCodecsByQuality = /* @__PURE__ */ new Map();
		...
		vodSelection = {
			...
			currentTrackPaths: [chosenVideo.baseUrl ?? chosenVideo.base_url, chosenAudio?.baseUrl ?? chosenAudio?.base_url].filter((item) => Boolean(item)).map(pathKeyFromUrl),
	...
	function handleLivePlayInfo(payload) {
		...
		const candidates = [];
		for (const stream of streams) for (const format of stream?.format ?? []) for (const codec of format?.codec ?? []) candidates.push({
			stream,
			format,
			codec
		});
		...
		const best = ranked[0];
		if (!best) return;
		const original = candidates.find((item) => item.stream.protocol_name === best.protocolName && item.format.format_name === best.formatName && item.codec.codec_name === best.codecName);
		if (!original) return;
	...
	function updateBuffered(video) {
		...
		if (buffered.length > 0) {
			...
			if (pageKind === "vod") {
				...
				if (bufferedEnd + 1 < lastBufferedRangeEnd) lastBufferedRangeEnd = bufferedEnd;
				const delta = bufferedEnd - lastBufferedRangeEnd;
				if (delta > .25 && delta < 20) {
					downloadController.avgVodSegmentSeconds = clampNumber(downloadController.avgVodSegmentSeconds * .75 + delta * .25, 1, 8);
					lastBufferedRangeEnd = bufferedEnd;
				} else if (delta >= 20) lastBufferedRangeEnd = bufferedEnd;
			}
		} else state.bufferedSeconds = 0;
		publishStatus();
	}
	function updateDroppedFrames(video) {
		state.droppedFrames = (video.getVideoPlaybackQuality?.())?.droppedVideoFrames ?? state.droppedFrames;
	}
	function maybeRecover(reason, video) {
		...
		if (now - (recoveryTimestamps.get(recoveryKey) ?? 0) < RECOVERY_LIMITS.sameActionCooldownMs) return;
	...
	async function executeRecovery(action, video) {
		...
		switch (action) {
			case "switch-live-protocol":
				requestPolicyPatch({ live: { preferredProtocol: nextLiveProtocolPreference() } });
				attemptPlayerMethod([
					"reload",
					"restart",
					"replay"
			...
			case "prefer-stable-codec": {
				requestPolicyPatch({ vod: { codecPreference: "avc" } });
				const currentQuality = currentPlayerQuality();
				if (state.quality != null && currentQuality !== state.quality) attemptPlayerMethod(["requestQuality", "setQuality"], state.quality);
				restorePlaybackPosition(video, restoreTarget);
				await ensurePlayback(video);
				return;
			}
			case "drop-quality": {
				...
				if (nextQuality != null) {
					requestPolicyPatch({
						...
						live: { preferredQuality: nextQuality }
					});
					state.quality = nextQuality;
					if (currentPlayerQuality() !== nextQuality) attemptPlayerMethod([
						"requestQuality",
						"setQuality",
						"switchQuality"
					], nextQuality);
	...
	function resolveRecoveryTargetTime(video) {
		if (seekState.active && Number.isFinite(seekState.targetTime)) return seekState.targetTime;
		if (seekState.recoveryAnchorExpiresAt != null && performance.now() <= seekState.recoveryAnchorExpiresAt && Number.isFinite(seekState.targetTime)) return seekState.targetTime;
		return Number.isFinite(video.currentTime) ? video.currentTime : null;
	}
	function restorePlaybackPosition(video, targetTime) {
	...
	function emitTelemetry(event) {
		...
		while (telemetry.length > policy.diagnostics.logLimit) telemetry.shift();
		window.postMessage({
	...
	function updateDerivedMetrics() {
		...
		state.avgSegmentDurationMs = durationSamples.length > 0 ? Math.round(durationSamples.reduce((sum, value) => sum + value, 0) / durationSamples.length) : 0;
		...
		state.recentThroughputMbps = downloadController.recentThroughputBps / 1e6;
	...
	function updateOverlay() {
		...
		if (!overlay) {
			...
			closeButton.addEventListener("click", () => {
				...
				requestPolicyPatch({ diagnostics: { overlayEnabled: false } });
				publishStatus();
			});
	...
	function installDownloadController() {
		window.setInterval(() => {
			refreshDownloadController();
			if (pageKind === "vod" && isPageActuallyHidden()) pumpVodBackgroundPrefetch();
			if (pageKind === "live" && isPageActuallyHidden()) maybeRefreshLivePlaylist();
			if (pageKind === "live" || prefetchQueue.length > 0 || prefetchActive > 0 || inflightRangeRequests.size > 0) publishStatus();
		}, 500);
	}
	function refreshDownloadController(force = false) {
		const now = Date.now();
		if (!force && now - downloadController.lastControllerTickMs < 500) return;
		...
		const recentThroughputBps = successful.reduce((sum, sample) => sum + sample.bytes, 0) / 2;
		...
		const recentBytesPerRequest = successful.length > 0 ? successful.reduce((sum, sample) => sum + sample.bytes, 0) / successful.length : 0;
	...
	function trimDownloadSamples(now) {
		while (downloadSamples.length > 0 && now - downloadSamples[0].ts > 2e3) downloadSamples.shift();
	}
	function resolveDownloadPhase() {
		if (pageKind === "live") return state.bufferedSeconds < 1.5 ? "liveUrgent" : "steady";
		if (seekState.active && !seekState.targetBuffered) return "seek";
		const video = document.querySelector("video");
		const currentTime = video && Number.isFinite(video.currentTime) ? video.currentTime : 0;
		if (segmentDurations.length < 2 || currentTime < 3 || state.bufferedSeconds < 4) return "initial";
		return "steady";
	}
	function resolveTargetBufferSeconds(phase) {
		if (phase === "liveUrgent") return 1.5;
		if (pageKind === "live") return policy.live.stableBufferTargetSeconds;
		const remainingSeconds = getVodPrefetchContext().remainingSeconds;
		if (remainingSeconds != null && remainingSeconds > 0) return Math.min(48, remainingSeconds);
		return 48;
	}
	...
	function resolveCacheTargetBytes(phase, windowSeconds) {
		if (pageKind === "live" || phase === "liveUrgent") return resolveLiveCacheTargetBytes({
			targetBufferSeconds: windowSeconds,
			avgSegmentSeconds: state.avgSegmentDurationMs > 0 ? state.avgSegmentDurationMs / 1e3 : null,
			recentBytesPerRequest: downloadController.recentBytesPerRequest,
			quality: state.quality
		});
		const bitrateEstimate = Math.max(25e5, vodSelection.bitrate ?? 5e6);
		const highBitrateMode = (state.quality ?? 0) >= 1440 || bitrateEstimate >= 8e6;
		const multiplier = phase === "seek" ? 2.1 : phase === "initial" ? 1.9 : 1.6;
	...
	function rewriteVodPayload(payload) {
		...
		if (policy.vod.lockQuality && policy.vod.preferredQuality != null) {
			...
			if (sameQuality.length > 0) candidates = sameQuality;
		}
		...
		const selected = candidates.find((item) => item.id === ranked[0]?.id && item.codecs === ranked[0]?.codecs) ?? candidates[0];
		if (!selected) return payload;
		dash.video = [selected];
		if (audios[0]) dash.audio = [audios[0]];
		return payload;
	}
	function rewriteLivePayload(payload) {
		const playurl = payload?.data?.playurl_info?.playurl;
		if (!playurl?.stream) return payload;
		const candidates = [];
		for (const stream of playurl.stream) for (const format of stream?.format ?? []) for (const codec of format?.codec ?? []) candidates.push({
			stream,
			format,
			codec
		});
		const selected = rankLiveCandidates(candidates)[0];
		if (!selected) return payload;
		const chosen = candidates.find((item) => item.stream.protocol_name === selected.protocolName && item.format.format_name === selected.formatName && item.codec.codec_name === selected.codecName);
		if (!chosen) return payload;
		playurl.stream = [{
			...chosen.stream,
			format: [{
				...chosen.format,
	...
	function rankLiveCandidates(candidates) {
		const filtered = candidates.filter((item) => {
			if (policy.live.preferredQuality != null && item.codec.current_qn !== policy.live.preferredQuality) return false;
			return true;
		});
		const pool = filtered.length > 0 ? filtered : candidates;
		return preferLiveProtocol(sortLiveCandidates(state.mode === "off" ? "stable" : state.mode, pool.map((item) => ({
			protocolName: item.stream.protocol_name ?? "",
			formatName: item.format.format_name ?? "",
			codecName: item.codec.codec_name ?? ""
		}))), policy.live.preferredProtocol);
	}
	function preferLiveProtocol(candidates, preferredProtocol) {
		if (preferredProtocol === "auto") return candidates;
		return [...candidates].sort((a, b) => {
			return Number(a.formatName !== preferredProtocol) - Number(b.formatName !== preferredProtocol);
		});
	}
	function resolveVodCodecPreference() {
		if (pageKind === "vod") return policy.vod.codecPreference;
		return "auto";
	}
	function scheduleVodPrefetch(currentUrl) {
		if (pageKind !== "vod" || state.mode === "off") return;
		...
		for (const task of buildVodPrefetchTasks(currentUrl, decision, context)) queuePrefetch(task);
	}
	function pumpVodBackgroundPrefetch(force = false) {
		if (pageKind !== "vod" || state.mode === "off" || !isPageActuallyHidden()) return;
		const now = Date.now();
		if (!force && now - lastVodBackgroundPumpAt < VOD_BACKGROUND_PUMP_INTERVAL_MS) return;
		const activeSegmentDownloads = prefetchActive + inflightRangeRequests.size;
		if (prefetchQueue.length + activeSegmentDownloads >= Math.max(1, downloadController.controllerAppliedConcurrency)) return;
		...
		if (cacheBytes > cacheTargetBytes + Math.max(8 * 1024 * 1024, cacheTargetBytes * .1)) return;
	...
	async function scheduleLivePlaylistPrefetch(playlistUrl, response) {
		if (pageKind !== "live" || state.mode === "off") return;
		lastLivePlaylistUrl = playlistUrl;
		const text = await response.text().catch(() => "");
		if (!text) return;
		const items = parseLivePlaylistItems(playlistUrl, text);
		noteLivePlaylistDurations(items.map((item) => item.durationSeconds).filter((item) => item != null));
		const mediaItems = items.filter((item) => item.kind === "media");
		const playlistItems = items.filter((item) => item.kind === "playlist");
		if (playlistItems.length > 0) lastLiveLeafPlaylistUrl = null;
		if (playlistItems.length === 0 && mediaItems.length > 0) lastLiveLeafPlaylistUrl = playlistUrl;
		...
		for (const item of playlistItems) {
			if (liveSeenSegments.has(item.url)) continue;
	...
	async function maybeRefreshLivePlaylist(force = false) {
		const playlistUrl = lastLiveLeafPlaylistUrl ?? lastLivePlaylistUrl;
		if (pageKind !== "live" || state.mode === "off" || !playlistUrl) return;
		if (inflightLivePlaylistRefresh) return inflightLivePlaylistRefresh;
		const activeSegmentDownloads = prefetchActive + inflightRangeRequests.size;
		const queueDepth = prefetchQueue.length + activeSegmentDownloads;
		const lowBuffer = state.bufferedSeconds < Math.max(2, policy.live.stableBufferTargetSeconds * .5);
		if (!force && !lowBuffer && queueDepth >= 3) return;
		const now = Date.now();
		if (!force && now - lastLivePlaylistRefreshAt < 1e3) return;
		...
		inflightLivePlaylistRefresh = (async () => {
			...
			if (!response?.ok) {
				...
				if (playlistUrl === lastLiveLeafPlaylistUrl) lastLiveLeafPlaylistUrl = null;
				emitNetworkEvent("playlist", playlistUrl, durationMs, false, void 0, "background-refresh");
				return;
			}
			noteHostSuccess(playlistUrl, durationMs);
			const bytesHeader = response.headers.get("content-length");
			emitNetworkEvent("playlist", playlistUrl, durationMs, true, bytesHeader ? Number(bytesHeader) : void 0, "background-refresh");
	...
	function getVodPrefetchContext() {
		...
		const durationSeconds = video && Number.isFinite(video.duration) && video.duration > 0 ? video.duration : vodSelection.durationSeconds;
		const remainingSeconds = durationSeconds != null ? Math.max(0, durationSeconds - (currentTime ?? 0)) : null;
		const resolvedPhase = resolveDownloadPhase();
		return {
			phase: resolvedPhase === "seek" || resolvedPhase === "initial" ? resolvedPhase : "steady",
			currentTime,
			durationSeconds,
			remainingSeconds
		};
	}
	function resolveVodBackgroundSeedUrl() {
		const candidates = [
			seekState.active && !seekState.targetBuffered ? lastMediaUrl : null,
	...
	function estimateVodSegmentDurationMs(_context) {
		return Math.round(resolveAvgVodSegmentSeconds() * 1e3);
	}
	function buildVodPrefetchTasks(currentUrl, decision, context) {
		...
		if (currentSequence != null) {
			...
			if (counterpartBaseUrl) {
				...
				if (counterpartCurrent) queueTask(tasks, counterpartCurrent, priority);
	...
	function appendTrackPrefetchTasks(tasks, baseUrl, windowCount, priority, boostCount) {
		...
		for (const [index, url] of futureUrls.entries()) queueTask(tasks, url, index < boostCount ? "seek" : priority);
	}
	function queueTask(tasks, url, priority) {
		if (tasks.some((task) => task.url === url)) return;
		tasks.push({
			url,
			kind: "media",
			priority
	...
	function queuePrefetch(task) {
		...
		if (mediaCache.has(normalized) || inflightPrefetches.has(normalized) || activeMediaFetches.has(normalized)) return;
		const existingIndex = prefetchQueue.findIndex((item) => item.url === normalized);
		if (existingIndex >= 0) {
			const existing = prefetchQueue[existingIndex];
			if (priorityScore(existing.priority) <= priorityScore(task.priority)) return;
	...
	function drainPrefetchQueue() {
		...
		while (prefetchActive < concurrency && prefetchQueue.length > 0) {
			...
			if (!task) break;
			const promise = runPrefetch(task).catch(() => null).finally(() => {
	...
	async function runPrefetch(task) {
		...
		if (rangeEntry) {
			...
			} else if (pageKind === "live" && isPageActuallyHidden()) maybeRefreshLivePlaylist();
			return rangeEntry;
		}
		const started = performance.now();
		...
		if (!response.ok) {
			...
			emitNetworkEvent("prefetch", task.url, durationMs, false, void 0, task.priority);
			recordDownloadSample({
				ts: Date.now(),
				ok: false,
				bytes: 0,
		...
		if (task.kind === "playlist") await scheduleLivePlaylistPrefetch(task.url, responseFromCache(entry).clone());
		else noteSegmentDuration(durationMs);
		if (pageKind === "vod" && task.kind === "media") {
			lastVodPrefetchSeedUrl = resolveNextVodSeedUrl(task.url, entry) || task.url;
			scheduleVodPrefetch(task.url);
		} else if (pageKind === "live" && task.kind === "media" && isPageActuallyHidden()) maybeRefreshLivePlaylist();
		return entry;
	}
	async function downloadPrefetchByRange(task) {
		...
		const buffers = await runRangeChunkTasks(tasks, void 0, resolveRangeSplitConcurrency(tasks[0]?.reason ?? "playback"));
		if (buffers.some((buffer) => buffer == null)) return null;
		const body = concatArrayBuffers(buffers);
		emitNetworkEvent("prefetch", task.url, 0, true, body.byteLength, `range:${tasks.length}`);
		return {
			url: task.url,
			body,
			status: 200,
	...
	async function probeRangeSupport(url, init) {
		...
		if (!acceptsRanges || !Number.isFinite(contentLength) || contentLength <= 0) return null;
		return {
			contentLength,
			headers: cloneHeaders(head.headers)
		};
	}
	function takeCachedEntry(url) {
	...
	function scheduleAfterFullBodyCacheHit(url, resourceKind, entry) {
		if (resourceKind === "media") {
			if (pageKind === "vod") {
				...
				} else lastVodPrefetchSeedUrl = url;
			} else if (pageKind === "live") maybeRefreshLivePlaylist();
			return;
		}
		if (resourceKind === "playlist") scheduleLivePlaylistPrefetch(url, responseFromCache(entry).clone());
	}
	function resolveNextVodSeedUrl(url, entry) {
		const sequence = extractUrlSequence(url);
		if (sequence != null) return replaceUrlSequence(url, sequence + 1) ?? url;
	...
	async function storeFetchedMediaResponse(url, response) {
		...
		const write = inflightEntry.then((entry) => {
			if (!entry || mediaCache.has(url)) return;
			storeCacheEntry(entry);
		}).finally(() => {
	...
	async function maybeServeMediaWithRangeSplit(url, input, init, semantics) {
		if (!shouldUseRangeSplit(url, semantics)) return null;
		const cacheKey = `${url}::seek=${seekState.active && !seekState.targetBuffered ? "1" : "0"}`;
		const inflight = inflightRangeRequests.get(cacheKey);
		if (inflight) return inflight.then((response) => response.ok && response.status === 200 ? response : null);
		const promise = runRangeSplitRequest(url, input, init).catch(() => null).finally(() => {
	...
	function shouldUseRangeSplit(url, semantics) {
	...
	async function runRangeSplitRequest(url, input, init) {
		const requestHeaders = cloneRequestHeaders(init?.headers);
		if (typeof Request !== "undefined" && input instanceof Request) input.headers.forEach((value, key) => {
		...
		const buffers = await runRangeChunkTasks(tasks, rangeInit, resolveRangeSplitConcurrency(tasks[0]?.reason ?? "playback"));
		if (buffers.some((buffer) => buffer == null)) return null;
	...
	async function fetchRangeChunk(task, init) {
		...
		try {
			...
			if (!response.ok || response.status !== 206) {
				...
				emitNetworkEvent("media", task.url, durationMs, false, void 0, `range:${task.start}-${task.end}`);
				recordDownloadSample({
					ts: Date.now(),
					ok: false,
					bytes: 0,
	...
	async function retryRangeChunk(task, init) {
		...
		if (!response || !response.ok || response.status !== 206) {
			noteHostFailure(task.url);
			noteHostCooldown(task.url);
			emitNetworkEvent("media", task.url, durationMs, false, void 0, `range-retry:${task.start}-${task.end}`);
			recordDownloadSample({
				ts: Date.now(),
				ok: false,
				bytes: 0,
	...
	function storeCacheEntry(entry) {
		if (mediaCache.has(entry.url)) {
			...
			if (previous) cacheBytes = Math.max(0, cacheBytes - previous.bytes);
		}
	...
	async function performFetchWithFallback(nativeFetch, input, init, _originalUrl, resourceKind) {
		...
		try {
			...
			if (response.ok || resourceKind !== "media" && resourceKind !== "playlist") return {
				response,
				finalUrl: firstUrl
			};
			...
			if (resourceKind !== "media" && resourceKind !== "playlist") throw error;
			noteHostFailure(firstUrl);
			throw error;
		}
	}
	...
	function noteHostSuccess(url, durationMs) {
		...
		if (health.cooldownUntil < now) health.cooldownUntil = 0;
		hostHealth.set(host, health);
	}
	...
	function recentHostFailures(windowMs = 9e4) {
		trimHostFailureSamples(Date.now(), windowMs);
		return hostFailureSamples.length;
	}
	function trimHostFailureSamples(now, windowMs = 9e4) {
		while (hostFailureSamples.length > 0 && now - hostFailureSamples[0].ts > windowMs) {
			...
			if (nextCount === 0) hostFailures.delete(host);
			else hostFailures.set(host, nextCount);
		}
	}
	function noteActiveMediaHost(url, resourceKind) {
		if (resourceKind !== "media" && resourceKind !== "playlist") return;
		lastMediaUrl = url;
		if (resourceKind === "media" || pageKind === "live") state.host = parseHost(url);
	}
	...
	function inferVodTrack(url) {
		...
		if (vodSelection.currentTrackPaths.some((item) => item === pathKey && /audio/i.test(item))) return "audio";
	...
	function nextLowerQuality(currentQuality) {
		if (currentQuality == null) return null;
		const currentIndex = vodSelection.availableQualities.findIndex((item) => item === currentQuality);
		if (currentIndex === -1 || currentIndex === vodSelection.availableQualities.length - 1) return null;
	...
	function nextLiveProtocolPreference() {
		...
		return current === "flv" ? "fmp4" : current === "fmp4" ? "flv" : state.protocol?.includes("flv") ? "fmp4" : "flv";
	}
	...
	function currentPlayerQuality() {
		...
		for (const object of objects) for (const method of ["getQuality", "getCurrentQuality"]) {
			const candidate = object[method];
	...
	function softSeek(video) {
		try {
			...
			video.currentTime = Math.max(0, video.currentTime - (pageKind === "live" ? 0 : .05));
		} catch {
			return;
		}
	}
	function noteSegmentDuration(durationMs) {
		...
		while (segmentDurations.length > 24) segmentDurations.shift();
	}
	function noteLivePlaylistDurations(durationsSeconds) {
		for (const durationSeconds of durationsSeconds) {
			if (!Number.isFinite(durationSeconds) || durationSeconds <= 0) continue;
			liveSegmentDurations.push(durationSeconds * 1e3);
		}
		while (liveSegmentDurations.length > 48) liveSegmentDurations.shift();
	}
	...
	function scheduleSeekPrefetch() {
		if (pageKind !== "vod") return;
		const seedUrl = lastMediaUrl ?? vodSelection.videoBaseUrl ?? vodSelection.audioBaseUrl;
		if (seedUrl) scheduleVodPrefetch(seedUrl);
	}
	function isTimeBuffered(video, time) {
		const ranges = video.buffered;
		for (let index = 0; index < ranges.length; index += 1) if (time >= ranges.start(index) && time <= ranges.end(index)) return true;
		return false;
	}
	function summarizeHostHealth() {
		const entries = [...hostHealth.entries()];
		if (entries.length === 0) return "";
		return entries.sort((a, b) => hostScore(b[1]) - hostScore(a[1])).slice(0, 3).map(([host, health]) => `${host}:${hostScore(health).toFixed(2)}`).join(", ");
	}
	function hostScore(health) {
		const total = health.successCount + health.failureCount;
		const successRatio = total === 0 ? 1 : health.successCount / total;
		const avgDuration = health.successCount === 0 ? 5e3 : health.totalDurationMs / health.successCount;
		const latencyFactor = Math.max(.1, Math.min(1, 2500 / Math.max(250, avgDuration)));
		const cooldownPenalty = health.cooldownUntil > Date.now() ? .2 : 1;
		return successRatio * latencyFactor * cooldownPenalty;
	}
	function isTargetQualitySatisfied() {
		if (pageKind === "vod") return (vodSelection.height ?? 0) >= 4320;
		if (pageKind === "live") return (state.quality ?? 0) >= 1e4;
		return false;
	}
	function classifyLiveBufferTier(bufferedSeconds) {
		const target = policy.live.stableBufferTargetSeconds;
		if (bufferedSeconds < Math.max(2, target * .5)) return "low";
		if (bufferedSeconds > target * 1.5) return "high";
		return "target";
	}
	...
	function getDocumentGetter(property) {
		const descriptor = Object.getOwnPropertyDescriptor(Document.prototype, property) ?? Object.getOwnPropertyDescriptor(Object.getPrototypeOf(document), property);
		return typeof descriptor?.get === "function" ? descriptor.get.bind(document) : null;
	}
	function defineDocumentGetter(property, getter) {
		try {
	...
	function isPageActuallyHidden() {
		...
		if (ownHiddenGetter && ownHiddenGetter !== spoofDocumentHidden) try {
			return Boolean(ownHiddenGetter.call(document));
		} catch {
			return false;
		}
		if (nativeDocumentHiddenGetter) try {
			return Boolean(nativeDocumentHiddenGetter());
		} catch {
			return false;
		}
		if (nativeDocumentVisibilityStateGetter) try {
	...
	function takeNextPrefetchTask() {
		if (!shouldBoostSeekPrefetch()) return prefetchQueue.shift();
	...
	function resolveRangeSplitConcurrency(reason) {
		...
		if (reason === "seek") return clampNumber(Math.round(configured / 2) + 2, 8, 12);
		return clampNumber(Math.round(configured / 2), 4, 8);
	}
	function detectPageKind(url) {
		if (/^https:\/\/live\.bilibili\.com\//.test(url)) return "live";
		if (/^https:\/\/www\.bilibili\.com\/video\//.test(url)) return "vod";
		return "unknown";
	}
	function buildStatus(partial, recentTelemetry) {
		const recent = recentTelemetry.filter((event) => event.ts >= Date.now() - 6e4);
